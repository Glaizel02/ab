...
[>] Panel: {current_mode}
    ...
    async def show_resource_management(self):
        ...
        table = Table(title=f"[38;5;189m]Resources[ ] Testing {len(resources)} entries...", show_header=True, header_style="bold cyan", width=50)
        table.add_column('#', style="dim", width=4)
        table.add_column("Type", width=8)
        table.add_column("Content", width=12)
        table.add_column("Status", width=12)
        table.add_column("Details", width=14)
        tested_resources = []
                ...
                if isinstance(resource, dict):
                    if 'cookie' in resource:
                        ...
                        if token:
                            ...
                            status = "[green]Valid" if test_result.get('valid') else "[red]Invalid"
                            details = f"Pages: {len(test_result.get('pages', []))}" if test_result.get('valid') else test_result.get('error', 'Unknown')
                        else:
                            status = "[red]Invalid"
                            details = "Token extraction failed"
                    elif 'token' in resource:
                        test_result = await self.verify_token(resource['token'])
                        status = "[green]Valid" if test_result.get('valid') else "[red]Invalid"
                        details = f"Pages: {len(test_result.get('pages', []))}" if test_result.get('valid') else test_result.get('error', 'Unknown')
                table.add_row(str(idx), resource.get('type', 'Unknown'), resource.get('token', resource.get('cookie', 'N/A'))[:10] + '...', status, details)
    ...
    async def show_token_generator(self):
        ...
        if choice == "1":
            ...
            if result["status"] == "success":
                output = f"Cookies: {result['cookies']}\nEAAAAU: {result['eaaau']}\nEAAD6V7: {result['eaad6v7']}\nEAAG: {result['eaag']}"
                if result["errors"]:
                    output += "\nErrors:\n" + "\n".join(result["errors"])
                self.print_panel("Tokens", output, "green")
                save = input("[›] Save to resources? (y/n): ")
                if save.lower() == 'y':
                    if result['cookies']:
                        self.db.add_resource({'cookie': result['cookies']})
                    if result['eaag']:
                        self.db.add_resource({'token': result['eaag']})
                    self.print_panel("Success", "Tokens saved!", "green")
            else:
                self.print_panel("Error", "\n".join(result["errors"]), "red")
        elif choice == "0":
            self.current_menu = "main"
    ...
    async def get_post_id(self, post_link: str) -> Optional[str]:
        ...
        try:
            patterns = [
                ...
                r'fbid=(\d+)',  # Added for photo/video fbid
                r'permalink\.php\?story_fbid=(\d+)&id=(\d+)',  # Added for permalink
                r'groups\/.+\/posts\/(\d+)',  # Added for group posts
            ]
