...
# Facebook share endpoint for web simulation
...
def shar():
    ...
    cookie_inputs = input(f"{green}☣ Enter Cookies (comma separated for multiple accounts): {gray}").split(',')
    ...
    cookies_list = [cookie_to_dict(c.strip()) for c in cookie_inputs if c.strip()]
    total_live = len(cookies_list)
    print(f"{green}☣ Live Cookies: {gray}{total_live}{reset}")
    if total_live == 0:
        print(f"{red}[!] No valid cookies entered. Exiting...{reset}")
        return
